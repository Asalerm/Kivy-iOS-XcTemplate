...
fileprivate class PythonRunTime {
    ...
    private func pythonHome() {
        ...
        putenv(python_home)
        let python_path = "PYTHONPATH=\(resourcePath):\(resourcePath)/lib/python\(PYTHON_VERSION)/:\(resourcePath)/lib/python\(PYTHON_VERSION)/site-packages:."
        putenv(python_path)
    }
